...
func TestRunRegulationScenario(t *testing.T) {
	...
	o.Group("TestRegulationScenario", func() {
		...
		o.Spec("TestShouldBanUserAfterTooManyAttempt", func(t *testing.T, s RodSuite) {
			...
			for i := 0; i < 3; i++ {
				err := s.WaitElementLocatedByCSSSelector(t, s.Context(ctx), "password-textfield").Input(badPassword)
				is.NoErr(err)
				err = s.WaitElementLocatedByCSSSelector(t, s.Context(ctx), "sign-in-button").Click("left")
				is.NoErr(err)
			}

			// Enter the correct password and test the regulation lock out
			err := s.WaitElementLocatedByCSSSelector(t, s.Context(ctx), "password-textfield").Input(testPassword)
			is.NoErr(err)
			err = s.WaitElementLocatedByCSSSelector(t, s.Context(ctx), "sign-in-button").Click("left")
			is.NoErr(err)
			s.verifyNotificationDisplayed(t, s.Context(ctx), "Incorrect username or password.")
