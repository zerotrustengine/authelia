...
func (rs *RodSession) doCompletePasswordReset(t *testing.T, page *rod.Page, newPassword1, newPassword2 string) {
	...
	rs.WaitElementLocatedByCSSSelector(t, page, "password1-textfield").MustInput(newPassword1)

	time.Sleep(1 * time.Second)
	rs.WaitElementLocatedByCSSSelector(t, page, "password2-textfield").MustInput(newPassword2)
